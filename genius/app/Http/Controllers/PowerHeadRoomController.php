...
class PowerHeadRoomController extends Controller
{
    ...
    {   
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_power_headroom where timeStamp <='$endTime' and timeStamp>='$startTime' and userLabel in $baseStation group BY timeStamp";
            } else {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_power_headroom where timeStamp >='$startTime' and timeStamp<='$endTime' group BY timeStamp";
            }
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            $result['total'] = $total;
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[65], "city" => $city, "date" => $timeStamp,"baseStationGroup"=>'baseStationGroup',"PHR满功率发射比例"=>$sum_avg), $temp);
            ...
            $sql = "select timeStamp,hourId,$searchArr,count(distinct ecgi) as count_ecgi from mrs_power_headroom where timeStamp >='$startTime' and timeStamp<='$endTime' ".$filter." group BY timeStamp,hourId";
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            $result['total'] = $total;
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[66],"city" => $city, "date" => $timeStamp, "hourId" => $hourId,"baseStationGroup"=>'baseStationGroup' ,/* "key"=>$key, */"PHR满功率发射比例"=>$sum_avg), $temp);
    ...
    {    
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_power_headroom where timeStamp >='$startTime' and timeStamp<='$endTime' and userLabel in $baseStation group BY timeStamp order by timeStamp";
            } else {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_power_headroom where timeStamp <='$endTime' and timeStamp>='$startTime' group BY timeStamp order by timeStamp";
            }
            $res = $db->query($sql);
            if ($res) {
                $rows = $res->fetchAll(PDO::FETCH_NUM);
                ...
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("date" => $timeStamp,"cellTotal"=>$row[65], "city" => $city, "baseStationGroup"=>'baseStationGroup',"PHR满功率发射比例"=>$sum_avg), $temp);
            ...
            $sql = "select timeStamp,hourId,$searchArr,count(distinct ecgi) as count_ecgi from mrs_power_headroom where timeStamp <='$endTime' and timeStamp >= '$startTime' ".$filter." group BY timeStamp,hourId order by timeStamp,hourId";
            $res = $db->query($sql);
            if ($res) {
                $rows = $res->fetchAll(PDO::FETCH_NUM);
                $city = $dbc->getPGToCHName($city);
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("date" => $timeStamp, "hourId" => $hourId,"cellTotal"=>$row[66], "city" => $city,"baseStationGroup"=>'baseStationGroup',"PHR满功率发射比例"=>$sum_avg), $temp);
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            $text = "date,cellTotal,city,baseStationGroup";
        } else if ($regionType == "baseStationGroup" && $timeType == "hour") {
            $text = "date,hourId,cellTotal,city,baseStationGroup";
