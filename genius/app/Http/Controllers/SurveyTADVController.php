...
class SurveyTADVController extends Controller
{
    ...
    {   
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_tadv where timeStamp <='$endTime' and timeStamp>='$startTime' and userLabel in $baseStation group BY timeStamp";
            } else {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_tadv where timeStamp >='$startTime' and timeStamp<='$endTime' group BY timeStamp";
            }
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            $result['total'] = $total;
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {                    
                    ...
                    $temp      = array();
                        ...
                        $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[46], "city" => $city,"终端平均距离"=>$ue_avg, "date" => $timeStamp,"baseStationGroup"=>'baseStationGroup'), $temp);
            ...
            $sql = "select timeStamp,hourId,$searchArr,count(distinct ecgi) as count_ecgi from mrs_tadv where timeStamp >='$startTime' and timeStamp<='$endTime' ".$filter." group BY timeStamp,hourId";
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            $result['total'] = $total;
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {                   
                    ...
                    for ($i = 2; $i <= ($sum_num+1); $i++) {
                        ...
                        $temp = array_merge(array("id" => $id++, "cellTotal"=>$row[47], "city" => $city, "终端平均距离"=>$ue_avg, "date" => $timeStamp, "hourId" => $hourId, "baseStationGroup"=>'baseStationGroup'), $temp);
    ...
    {    
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    for ($i = 1; $i <= ($sum_num); $i++) {
                        ...
                        $temp = array_merge(array("date" => $timeStamp, "cellTotal"=>$row[46], "city" => $city, "终端平均距离"=>$ue_avg, "baseStationGroup"=>'baseStationGroup'), $temp);
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    if ($i-2<10) {
                        ...
                        $temp = array_merge(array("date" => $timeStamp, "hourId" => $hourId, "cellTotal"=>$row[47], "city" => $city, "终端平均距离"=>$ue_avg, "baseStationGroup"=>'baseStationGroup'), $temp);
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            $text = "date,cellTotal,city,终端平均距离,baseStationGroup";
        } else if ($regionType == "baseStationGroup" && $timeType == "hour") {
            $text = "date,hourId,cellTotal,city,终端平均距离,baseStationGroup";
