...
class CoverageQueryController extends Controller
{
    ...
    public function getTableData()
    {
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$RSRP,count(distinct ecgi) as count_ecgi from mrs_rsrp where timeStamp <='$endTime' and timeStamp>='$startTime' and userLabel in $baseStation group BY timeStamp";
            } else {
                $sql = "select timeStamp,$RSRP,count(distinct ecgi) as count_ecgi from mrs_rsrp where timeStamp <='$endTime' and timeStamp>='$startTime' group BY timeStamp";
            }
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[49],"city" => $city, "date" => $timeStamp, "baseStationGroup"=>'baseStationGroup',"总RSRP采样点总数"=>$sum_RSRP, "RSRP平均覆盖率"=>$result['avg_RSRP'], "RSRP的比例"=>$result['avgs']), $temp);
            ...
            $sql = "select timeStamp,hourId,$RSRP,count(distinct ecgi) as count_ecgi from mrs_rsrp where timeStamp>='$startTime' and timeStamp <= '$endTime' ".$filter." group BY timeStamp,hourId";
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[50], "city" => $city, "date" => $timeStamp, "hourId" => $hourId,"baseStationGroup"=>'baseStationGroup' ,"总RSRP采样点总数"=>$sum_RSRP,"RSRP平均覆盖率"=>$result['avg_RSRP'],"RSRP的比例"=>$result['avgs']), $temp);
    ...
    public function exportFile()
    {
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$RSRP,count(distinct ecgi) as count_ecgi from mrs_rsrp where timeStamp>='$startTime' and timeStamp<='$endTime' and userLabel in $baseStation group BY timeStamp order by timeStamp";
            } else {
                $sql = "select timeStamp,$RSRP,count(distinct ecgi) as count_ecgi from mrs_rsrp where timeStamp>='$startTime' and timeStamp<='$endTime' group BY timeStamp order by timeStamp";
            }
            $res = $db->query($sql);
            if ($res) {
                $rows = $res->fetchAll(PDO::FETCH_NUM);
                ...
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("date" => $timeStamp,"cellTotal"=>$row[49], "city" => $city,"baseStationGroup"=>'baseStationGroup',"总RSRP采样点总数"=>$sum_RSRP,"RSRP平均覆盖率"=>$result['avg_RSRP'],"RSRP的比例"=>$result['avgs']), $temp);
            ...
            $sql = "select timeStamp,hourId,$RSRP,count(distinct ecgi) as count_ecgi from mrs_rsrp where timeStamp >='$startTime' and timeStamp<='$endTime' ".$filter." group BY timeStamp,hourId order by timeStamp,hourId";
            $res = $db->query($sql);
            if ($res) {
                $rows = $res->fetchAll(PDO::FETCH_NUM);
                $city = $dbc->getPGToCHName($city);
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("date" => $timeStamp, "hourId" => $hourId,"cellTotal"=>$row[50], "city" => $city,"baseStationGroup"=>'baseStationGroup',"总RSRP采样点总数"=>$sum_RSRP,"RSRP平均覆盖率"=>$result['avg_RSRP'],"RSRP的比例"=>$result['avgs']), $temp);
            ...
            $sql = "select timeStamp,hourId,userLabel,$RSRP,count(distinct ecgi) as count_ecgi from mrs_rsrp where timeStamp >='$startTime' and timeStamp<='$endTime' ".$filter." group BY timeStamp,hourId,userLabel order by timeStamp,userLabel,hourId";
            $res = $db->query($sql);
            if ($res) {
                $rows = $res->fetchAll(PDO::FETCH_NUM);
                $city = $dbc->getPGToCHName($city);
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            $text = "date,cellTotal,city,baseStationGroup,总RSRP采样点总数,RSRP平均覆盖率,RSRP>=-110的比例";
        } else if ($regionType == "baseStationGroup" && $timeType == "hour") {
            $text = "date,hourId,cellTotal,city,baseStationGroup,总RSRP采样点总数,RSRP平均覆盖率,RSRP>=-110的比例";
