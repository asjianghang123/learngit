...
class SurveySinrController extends Controller
{
    ...
    {   
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_sinrul where timeStamp <='$endTime' and timeStamp>='$startTime' and userLabel in $baseStation group BY timeStamp";
            } else {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_sinrul where timeStamp >='$startTime' and timeStamp<='$endTime' group BY timeStamp";
            }
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[38], "city" => $city, "total"=>$result['total'],"sinr"=>$result['sinr'],"sinr_avg"=>$result['sinr_avg'],"total_avg"=>$result['total_avg'], "date" => $timeStamp,"baseStationGroup"=>'baseStationGroup'), $temp);
            ...
            $sql = "select timeStamp,hourId,$searchArr,count(distinct ecgi) as count_ecgi from mrs_sinrul where timeStamp >='$startTime' and timeStamp<='$endTime' ".$filter." group BY timeStamp,hourId";
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $result=$this->getueAvg($temp);
                        $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[39], "city" => $city, "total"=>$result['total'],"sinr"=>$result['sinr'],"sinr_avg"=>$result['sinr_avg'],"total_avg"=>$result['total_avg'], "date" => $timeStamp, "hourId" => $hourId,"baseStationGroup"=>'baseStationGroup'), $temp);
    ...
    {    
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_sinrul where timeStamp >='$startTime' and timeStamp<='$endTime' and userLabel in $baseStation group BY timeStamp order by timeStamp";
            } else {
                $sql = "select timeStamp,$searchArr,count(distinct ecgi) as count_ecgi from mrs_sinrul where timeStamp <='$endTime' and timeStamp>='$startTime' group BY timeStamp order by timeStamp";
            }
            $res = $db->query($sql);
            if ($res) {
                $rows = $res->fetchAll(PDO::FETCH_NUM);
                ...
                $temp = array_merge(array("date" => $timeStamp, "city" => $city, "baseStationGroup"=>'baseStationGroup',"total"=>$result['total'],"sinr"=>$result['sinr'],"sinr_avg"=>$result['sinr_avg'],"total_avg"=>$result['total_avg']), $temp);
            ...
            $sql = "select timeStamp,hourId,$searchArr,count(distinct ecgi) as count_ecgi from mrs_sinrul where timeStamp <='$endTime' and timeStamp >= '$startTime' ".$filter." group BY timeStamp,hourId order by timeStamp,hourId";
            $res = $db->query($sql);
            if ($res) {
                $rows = $res->fetchAll(PDO::FETCH_NUM);
                $city = $dbc->getPGToCHName($city);
                foreach ($rows as $row) {
                    ...
                    $temp = array_merge(array("date" => $timeStamp, "hourId" => $hourId, "city" => $city,"baseStationGroup"=>'baseStationGroup', "total"=>$result['total'],"sinr"=>$result['sinr'],"sinr_avg"=>$result['sinr_avg'],"total_avg"=>$result['total_avg']), $temp);
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            $text = "date,city,baseStationGroup,采样点总数,SINR<-3采样点数,SINR<-3的比例,平均上行SINR";
        } else if ($regionType == "baseStationGroup" && $timeType == "hour") {
            $text = "date,hourId,city,baseStationGroup,采样点总数,SINR<-3采样点数,SINR<-3的比例,平均上行SINR";
