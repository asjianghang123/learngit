...
/**
 * ConsistencyCheckController.php
 *
...
/**
 * 一致性检查
 * Class ConsistencyCheckController
 *
 * @category ParameterAnalysis
 * @package  App\Http\Controllers\ParameterAnalysis
 * @author   ericsson <genius@ericsson.com>
...
class ConsistencyCheckController extends Controller
{
    ...
    {
        $rows = Databaseconns::groupBy('cityChinese')->get()->toArray();
        $cityArr = array();
        foreach ($rows as $row) {
            array_push($cityArr, $row['cityChinese']);
        }
        return $cityArr;
    }
    ...
    public function getDistributeData()
    {
        ...
        for ($i = 0; $i < count($tables); $i++) {
            ...
            for ($j = 0; $j < count($YCategories); $j++) {
                $cityCh = $YCategories[$j];
                $city = $dbc->getCityByCityChinese($cityCh)[0]->connName;
                $subNetwork = $dbc->getSubNets($cityCh);
                if ($table == 'EUtranCellRelationUnidirectionalNeighborCell' || $table == 'EUtranCellRelationExistNeighborCellWithoutX2' || $table == 'EUtranCellRelationManyNeighborCell' || $table == 'EUtranCellRelationFewNeighborCell' || $table == 'EUtranCellRelationOverDistanceCell' || $table == 'EUtranCellRelationNeighOfRSI') {
                    $sql = "select count(distinct EUtranCell) as occurs from $table t where city='".$city."'";
                }elseif ( $table == 'MeasuringFrequencyTooMuch') {
                    $sql = "select count(distinct EUtranCell) as occurs from $table t where subNetwork in(" . $subNetwork . ")";
                }
    ...
    public function getItems()
    {
        ...
        $city = $dbc->getCityByCityChinese($cityCh)[0]->connName;
        $subNetwork = $dbc->getSubNets($cityCh);
        if ($table == 'EUtranCellRelationUnidirectionalNeighborCell') {
            if ($subNetwork != '') {
                $filter = " where city='".$city."' and EUtranCell not in (select EUtranCellTDD from mongs.UnidirectionalNeighborCell_Template where subNetwork in(" . $subNetwork . "))";
            } 
            else {
                $filter = " where EUtranCell not in (select EUtranCellTDD from mongs.UnidirectionalNeighborCell_Template)";
            }
        } 
        else if($table == 'TempParameterKgetExternalCompare_1'){
    ...
    public function exportTemplate()
    {
        ...
        if ($table == 'EUtranCellRelationUnidirectionalNeighborCell') {
            ...
            $defaultFileName = "common/template/UnidirectionalNeighborCell_Template.csv";
        } else if ($table == 'TempEUtranCellFreqRelation') {
            $templateTable = 'TempEUtranCellFreqRelationWhiteList';
            $filter = " where city=:city ";
            $defaultFileName = "common/template/TempEUtranCellFreqRelationWhiteListTemplate.csv";
        } else {
