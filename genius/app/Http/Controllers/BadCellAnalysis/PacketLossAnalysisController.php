...
class PacketLossAnalysisController extends MyRedis
{
    ...
    public function getTableData()
    {
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$ULQciX,$DLQciX, count(distinct ecgi) as count_ecgi from mrs_packet_lossrate where timeStamp>='$startTime' and timeStamp <= '$endTime' and userLabel in $baseStation group BY timeStamp";
            } else {
                $sql = "select timeStamp,$ULQciX,$DLQciX, count(distinct ecgi) as count_ecgi from mrs_packet_lossrate where timeStamp>='$startTime' and timeStamp <= '$endTime' group BY timeStamp";
            }
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $baseStationGroup='baseStationGroup';
                            $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[57],"city" => $city, "date" => $timeStamp,"baseStationGroup"=>$baseStationGroup, "上行丢包率" => $up, "下行丢包率" => $down), $temp);
            ...
            $sql = "select timeStamp,hourId,$ULQciX,$DLQciX,count(distinct ecgi) as count_ecgi from mrs_packet_lossrate where timeStamp>='$startTime' and timeStamp <= '$endTime' ".$filter." group BY timeStamp,hourId";
            $res = $db->query("select count(*) from ($sql) as total");
            $total = $res->fetchAll(PDO::FETCH_NUM)[0];
            $result['total'] = $total;
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $baseStationGroup='baseStationGroup';
                    $temp = array_merge(array("id" => $id++,"cellTotal"=>$row[58],"city" => $city, "date" => $timeStamp, "hourId" => $hourId,"baseStationGroup"=>$baseStationGroup,"上行丢包率" => $up, "下行丢包率" => $down), $temp);
    ...
    public function exportFile()
    {
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            if ($baseStationStr) {
                $sql = "select timeStamp,$ULQciX,$DLQciX,count(distinct ecgi) as count_ecgi from mrs_packet_lossrate where timeStamp>='$startTime' and timeStamp <= '$endTime' and userLabel in $baseStation group BY timeStamp order by timeStamp";
            } else {
                $sql = "select timeStamp,$ULQciX,$DLQciX,count(distinct ecgi) as count_ecgi from mrs_packet_lossrate where timeStamp>='$startTime' and timeStamp <= '$endTime' group BY timeStamp order by timeStamp";
            }
            ...
            if ($res) {
                ...
                foreach ($rows as $row) {
                    ...
                    $baseStationGroup='baseStationGroup';
                    $temp = array_merge(array("date" => $timeStamp,"cellTotal"=>$row[57],"city" => $city, "baseStationGroup"=>$baseStationGroup, "上行丢包率" => $up, "下行丢包率" => $down), $temp);
            ...
            $sql = "select timeStamp,hourId,$ULQciX,$DLQciX,count(distinct ecgi) as count_ecgi from mrs_packet_lossrate where timeStamp>='$startTime' and timeStamp <= '$endTime' ".$filter." group BY timeStamp,hourId order by timeStamp,hourId";
            $res = $db->query($sql);
            if ($res) {
                $rows = $res->fetchAll(PDO::FETCH_NUM);
                $city = $dbc->getPGToCHName($city);
                foreach ($rows as $row) {
                    ...
                    $baseStationGroup='baseStationGroup';
                    $temp = array_merge(array("date" => $timeStamp, "hourId" => $hourId,"cellTotal"=>$row[58], "city" => $city,"baseStationGroup"=>$baseStationGroup, "上行丢包率" => $up, "下行丢包率" => $down), $temp);
        ...
        if ($regionType == "city" && $timeType == "day") {
            ...
            $text = "date,cellTotal,city,baseStationGroup,上行丢包率,下行丢包率";
        } else if ($regionType == "baseStationGroup" && $timeType == "hour") {
            $text = "date,hourId,cellTotal,city,baseStationGroup,上行丢包率,下行丢包率";
        } else if ($regionType == "baseStation" && $timeType == "hour") {
