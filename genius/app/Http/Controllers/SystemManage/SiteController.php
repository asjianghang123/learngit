...
class SiteController extends Controller
{
    ...
    public function querySite4G()
    {
        ...
        if ($value == "city") {
            $row = $conn->select(DB::raw('HIGH_PRIORITY *'))->orderBy('id', 'asc')->paginate($rows)->toArray();
        } else {
            $row = $conn->select(DB::raw('HIGH_PRIORITY *'))->where('city', $value)->orderBy('id', 'asc')->paginate($rows)->toArray();
        }
    ...
    public function querySite3G()
    {
        ...
        $rows = isset($_REQUEST['limit']) ? intval($_REQUEST['limit']) : 50;
        if ($value == "city") {
            $row = SiteCdma::select(DB::raw('HIGH_PRIORITY *'))->paginate($rows)->toArray();
        } else {
            $row = SiteCdma::select(DB::raw('HIGH_PRIORITY *'))->where('city', 'like', $value . "%")->orderBy('id', 'asc')->paginate($rows)->toArray();
        }
        $result["total"] = $row['total'];
    ...
    public function downloadFile()
    {
        ...
        if ($table == 'siteManage') {
            $column = 'HIGH_PRIORITY ecgi,ECI,cellName,cellNameChinese,siteName,siteNameChinese,duplexMode,rsi,tac,longitude,latitude,dir,pci,earfcn,siteType,cellType,tiltM,tiltE,antHeight,dualBandNetwork,CANetwork,address,band,channelBandWidth,`noofTxAntennas(Site)`,highTraffic,highInterference,HST,cluster,subNetwork,currentOSS,覆盖属性,是否在网,city,importDate';
            if ($searchText) {
                $searchText = "%".$searchText."%";
                $conn = SiteLte::where(function($query) use ($searchText) {
                    $query->where('cellName', 'like', $searchText)->orWhere('ecgi', 'like', $searchText);
            ...
            $column = 'HIGH_PRIORITY CELL,CellNameChinese,CellIdentity,BAND,ARFCN,Longitude,Latitude,dir,cellType,height,plmnIdentity_mcc,plmnIdentity_mnc,LAC,BCCH,BCC,NCC,dtmSupport,city,importDate ';
            if ($searchText) {
                $searchText = "%".$searchText."%";
                $conn = SiteGsm::where('CELL', 'like', $searchText);
            } else {
            ...
            $column = 'HIGH_PRIORITY ecgi,cellName,cellNameChinese,siteName,siteNameChinese,tac,longitude,latitude,dir,pci,earfcn,siteType,cellType,tiltM,tiltE,antHeight,dualBandNetwork,band,channelBandWidth,`noofTxAntennas(Site)`,cluster,覆盖属性,厂商,city,importDate';
            if ($searchText) {
                $searchText = "%".$searchText."%";
                $conn = OtherSiteLte::where(function($query) use ($searchText) {
                    $query->where('cellName', 'like', $searchText)->orWhere('ecgi', 'like', $searchText);
            ...
            $column = 'HIGH_PRIORITY Cell,SiteNo,Name,Area,MSC,RNC,RNCID,cid,LAC,PSC,uarfcnDl,type,`indoor/outdoor`,Longitude,Latitude,Address,Antennaaddress,3Gantennaheight,3Gantennadirection,`Federlength()`,3Gantennatype,`3GantennaH-beam`,`3GantennaV-beam`,`3GE-tiltrange`,`3GantennaE-tilt`,`3GantennaM-tilt`,dualbandantennawithGSM,dualbandantennawithDCS,`Co-siteGSMsite`,`Co-siteGSMsitename`,GSMantennaheight,GSMantennadirection,GSMantennatilt,`Co-siteDCSsite`,`Co-siteDCSsitename`,DCSantennaheight,DCSantennadirection,DCSantennatilt,city,importDate';
            if ($city == 'city') {
        ...
        //end if
        $fileUtil = new FileUtil();
        $fileUtil->resultToCSV2(str_replace("HIGH_PRIORITY ","",$column), $items, $fileName);
