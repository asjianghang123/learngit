...
function initConsistencyTree() {
	...
	$.post("consistencyCheck/getCityList", null, function (cityData) {
		YCategories = cityData;
		data_city   = cityData;
			...
			var options = {
				...
				onNodeSelected: function (event, data) {
					...
					if (data.value == "S1Related") {
						...
						tables = ["ExternalEUtranCellActivePlmnListCheck", "TermPointToMme_S1_MMEGI_dif", "TermPointToMme_S1_UsedIP"];
						...
						/*XCategories = ["冗余频点","单向邻区","有邻区无X2","邻区过多","邻区过少","2G邻区过少","测量频点过多核查","冗余GeranFrequency频点核查"];
						 tables = ["TempEUtranCellFreqRelation","EUtranCellRelationUnidirectionalNeighborCell","TempEUtranCellRelationExistNeighborCellWithoutX2","TempEUtranCellRelationManyNeighborCell","TempEUtranCellRelationFewNeighborCell","TempGeranCellRelation2GNeighbor","TempMeasuringFrequencyTooMuch","TempGeranFrequency_1_check"];*/
						XCategories = ["单向邻区", "有邻区无X2", "邻区过多", "邻区过少","超远距离邻区核查","同频RSI冲突核查"];
						tables = ["EUtranCellRelationUnidirectionalNeighborCell", "EUtranCellRelationExistNeighborCellWithoutX2", "EUtranCellRelationManyNeighborCell", "EUtranCellRelationFewNeighborCell","EUtranCellRelationOverDistanceCell","EUtranCellRelationNeighOfRSI"];
						...
						XCategories = ["2G外部定义核查", "4G外部定义核查", "冗余External_4G定义核查"];
						tables = ["TempParameter2GKgetCompare", "TempExternalNeigh4G", "redundancy_External4G"];
						...
						tables = ["TempEUtranCellFreqRelation", "TempMissingMRFrequency", "MeasuringFrequencyTooMuch", "TempMissEqualFrequency", "TempGeranFrequency_1_check", "TempGeranFrequencyException"];
						...
						events: {
							click: function (e) {
								...
								if (table == "ossInfo") {
									...
									$("#TempEUtranCellFreqRelation").css("display", "none");
									consistencyCheckDetailsSearch_1(params);
								} else if (table == "EUtranCellRelationUnidirectionalNeighborCell") {
									$("#searchType").css("display", "none");
									$("#checkType").css("display", "none");
									...
									$("#LatlonCheckWhiteList").css("display", "none");
									$("#TempEUtranCellFreqRelation").css("display", "none");
									tableRow = "EUtranCellRelationUnidirectionalNeighborCell";
									// $("#tableName").val("EUtranCellRelationUnidirectionalNeighborCell");
...
function importFile() {
	...
	var whiteListTable = "";
	if (tableRow == "EUtranCellRelationUnidirectionalNeighborCell") {
